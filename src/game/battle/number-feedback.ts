...
export type NumberFeedback = { target: string; kind: string; source: string; amount: number; suffix: string; reducedMotion: boolean };
...
/** Aggregate only explicitly identified, same-source numeric effects in a short
...
export function presentNumberFeedback(scene: Phaser.Scene, layer: Phaser.GameObjects.Container,
  ...
  const enemy = scene.children.list.flatMap(object => 'list' in object ? (object as Phaser.GameObjects.Container).list : [object])
    .some(object => object.name === 'combat-enemy-art' && object.getData('enemyId') === value.target);
  ...
  const update = (entry: NumberBurst) => {
    entry.label.setText(`${entry.amount}${value.suffix}${entry.count > 1 ? ' total' : ''}`);
    entry.plate.setSize(Math.max(120, entry.label.width + 24), 34);
    entry.group.x = Phaser.Math.Clamp(x, entry.plate.width / 2 + 8, scene.scale.width - entry.plate.width / 2 - 8);
  ...
  const group = scene.add.container(x, enemy ? 405 : Phaser.Math.Clamp(y, 220, 420) - slot * 40).setName('combat-number-feedback');
